...
async function getBinanceOptionsInfo(expiryDate, time) {
    ...
    try {
        ...
        console.log('Error during options gathering information: \n' + err)
    }
}
...
async function getBybitOptionsInfo(expiryDate, time)  {
    ...
    try {
        const response = await fetch(bybitUrl + instrumentInfoEndpoint + new URLSearchParams(instrumentParams))
        ...
        console.log('Error during Bybit fetcing instruments info: ' + err)
    }
    ...
    try {
        ...
        const response = await fetch(bybitUrl + endpoint + new URLSearchParams(params))
        ...
        console.log("Error during underlyingPrice fetching: " + err)
    }
    ...
    for (let i = 0; i < 3; i++) {
        ...
        try {
            ...
            console.log('Error during Bybit options tickers info fetching: ' + err)
        }
